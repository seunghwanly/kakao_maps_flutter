...
public class GuiLayout extends GuiView {
    ...
    public void setBackground(int resourceId, boolean isNinepatch) {
        this.background = new GuiImage(resourceId, isNinepatch);
    }
    ...
    public void setBackground(GuiImage image) {
    ...
    public void addView(GuiView view) {
        if (this.childArray != null && this.childArray.length > 0) {
            List<GuiView> temp = new ArrayList();
            temp.addAll(Arrays.asList(this.childArray));
            temp.add(view);
            this.childArray = (GuiView[])temp.toArray(new GuiView[temp.size()]);
        } else {
            this.childArray = new GuiView[1];
            this.childArray[0] = view;
        }

    }

    public boolean hasChild() {
        return this.childArray != null && this.childArray.length > 0;
    }
    ...
    public GuiView getChildAt(int index) {
        return this.childArray == null ? null : this.childArray[index];
    }

    public GuiView getChild(String id) {
        if (this.childArray == null) {
            return null;
        } else {
            for(GuiView child : this.childArray) {
                if (child.getId() == id) {
                    return child;
                }
            }
